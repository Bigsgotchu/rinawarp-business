...
## A) Soft Lock (Inline, when user hits AI-only feature)
...
## B) Contextual Nudge (After 2–3 heuristic suggestions)
...
## C) Grace / Past Due State (Non-punitive)
...
## D) Pricing Clarity (No surprises)
...
- Always mention local agent availability
...
// Grace state trigger
if (subscriptionStatus === 'past_due' && gracePeriodActive) {
  showUpsellModal('grace-state');
}
```
...
- **Positioning**: Modal vs inline vs sidebar
