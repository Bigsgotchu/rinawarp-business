...
export const Billing: React.FC = () => {
  ...
  const [customerDetail, setCustomerDetail] = useState<BillingCustomerDetail | null>(null);
  ...
  async function fetchCustomerByEmail(e: React.FormEvent) {
    ...
    try {
      const url = new URL('/api/admin/billing-customer', window.location.origin);
  ...
  if (!hasToken) {
    return (
      <div className="p-6 text-sm text-neutral-300">
        <h1 className="text-xl font-semibold mb-2">Billing Overview</h1>
        <p>
          Set your admin API token in the <strong>Settings</strong> page to access billing data.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6 text-neutral-100">
      <div>
        <h1 className="text-2xl font-semibold mb-1">Billing Overview</h1>
        <p className="text-sm text-neutral-400">
          Unified view of purchases, subscriptions, and credits from Stripe + KV.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-xl border border-neutral-700 bg-neutral-900/60 p-4">
          <div className="text-xs text-neutral-400">Recent Revenue</div>
          <div className="text-xl font-semibold mt-1">
            {summary ? formatCurrency(summary.summary.recentRevenueCents, 'usd') : '—'}
          </div>
        </div>
        <div className="rounded-xl border border-neutral-700 bg-neutral-900/60 p-4">
          <div className="text-xs text-neutral-400">Recent Purchases</div>
          <div className="text-xl font-semibold mt-1">
            {summary?.summary.recentPurchasesCount ?? '—'}
          </div>
        </div>
        <div className="rounded-xl border border-neutral-700 bg-neutral-900/60 p-4">
          <div className="text-xs text-neutral-400">Active Subscriptions</div>
          <div className="text-xl font-semibold mt-1">
      ...
      <div className="rounded-xl border border-neutral-700 bg-neutral-900/60 p-4 space-y-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            <div className="text-sm font-medium">Look up customer by email</div>
        ...
        <form onSubmit={fetchCustomerByEmail} className="flex flex-col sm:flex-row gap-2 mt-2">
          <input
            type="email"
          ...
          <div className="mt-4 space-y-4">
            <div>
              <div className="text-sm font-semibold">
                {customerDetail.customer?.email || customerDetail.customerId}
              </div>
              <div className="text-xs text-neutral-400">
                Stripe Customer ID: {customerDetail.customerId || '—'}
              </div>
              {customerDetail.customer && (
                <div className="mt-1 text-xs text-neutral-400">
                  LTV: {formatCurrency(customerDetail.customer.ltvCents || 0, 'usd')}
                  {' · '}Last seen: {customerDetail.customer.lastSeenAt || '—'}
                </div>
              )}
            </div>
                    ...
                    {customerDetail.purchases.map((p) => (
                      <tr key={p.id} className="border-t border-neutral-800/70">
                        ...
                        <td className="px-3 py-2">
                          {p.productType} {p.productId ? `· ${p.productId}` : ''}
                        </td>
                        <td className="px-3 py-2 text-right">
                          {formatCurrency(p.amountTotalCents, p.currency || 'usd')}
                        </td>
                      </tr>
                    ))}
            ...
            <div>
              ...
              {customerDetail.credits ? (
                <div className="text-xs text-neutral-300">
                  Remaining:{' '}
                  <span className="font-semibold">{customerDetail.credits.remaining}</span> · Last
                  updated: {customerDetail.credits.lastUpdated || '—'}
                </div>
              ) : (
                <div className="text-xs text-neutral-500">No credits record found.</div>
              )}
            </div>
          </div>
        )}
              ...
              {!summary?.purchases?.length && (
                <tr>
                  <td colSpan={5} className="px-3 py-3 text-center text-neutral-500">
                    No purchase data yet. Once Stripe starts sending webhooks, events will appear
                    here.
                  </td>
                </tr>
              )}
