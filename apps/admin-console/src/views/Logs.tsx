...
export const Logs: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadLogs = async () => {
      ...
      setError(null);
      try {
        const data = await getLogs(apiToken);
        setLogs(data);
      } catch (err) {
    ...
    <div>
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden text-xs">
        <div className="max-h-[480px] overflow-auto font-mono">
          {logs.map((log) => (
            <div key={log.id} className="border-b border-neutral-800 px-3 py-2 flex gap-2">
              <div className="text-neutral-500 min-w-[145px]">
                {new Date(log.timestamp).toLocaleString()}
              </div>
              <div className="text-neutral-400 min-w-[170px]">{log.source}</div>
              <div
                className={
                  'min-w-[60px] ' +
                  (log.level === 'error'
          ...
          {!logs.length && <div className="px-3 py-3 text-neutral-500">No logs received yet.</div>}
        </div>
      </div>

      {loading && <div className="mt-3 text-xs text-neutral-500">Loading logs…</div>}
    </div>
  );
};
