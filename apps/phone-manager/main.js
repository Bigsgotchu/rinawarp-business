...
ipcMain.handle('pull-file', async (event, deviceId, remotePath, localPath, platform) => {
  console.log(
    `Pulling ${remotePath} to ${localPath} on device ${deviceId} (${platform}).`
  )
...
ipcMain.handle('backup-device', async (event, payload = {}) => {
  const {
    ...
    platform,
    destination,
    paths = DEFAULT_ANDROID_BACKUP_PATHS
  } = payload
...
ipcMain.handle(
  ...
  async (event, deviceId, platform, command) => {
      ...
      if (platform === 'android') {
        ...
        if (command.includes('testdisk')) {
          return {
            success: true,
            output:
              'iOS MOCK: Ran privileged testdisk binary. Output: Found 5 partitions.'
          }
        }
        throw new Error(
          'iOS privileged execution requires SSH access on a jailbroken device.'
        )
      }
...
ipcMain.handle('trigger-secure-unlock', async (event, deviceId, proofFilePath) => {
  console.log(
    `Attempting secure unlock for ${deviceId} with proof file: ${proofFilePath}`
  )
