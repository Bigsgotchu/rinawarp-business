...
export async function handleChat(messages, model) {
  try {
    ...
    if (
      userInput.trim().startsWith('$') ||
      userInput.toLowerCase().includes('execute') ||
      userInput.toLowerCase().includes('run command')
    ) {
      try {
