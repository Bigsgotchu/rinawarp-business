...
export function planNextStep(ctx: PlanningContext): NextStep {
  ...
  if (ctx.lastShell && isBuildCommand(ctx.lastShell.cmd) && ctx.lastShell.exitCode !== 0) {
    rules.push({
      ...
      next: {
        ...
        tool: {
          tool: 'shell.run',
          input: { cmd: "echo 'Check terminal output above for specific error details'" },
        },
      },
    });
  }
    ...
    rules.push({
      ...
      next: {
        ...
        items: [
          {
            label: 'Review status',
            acceptText: 'git status',
            tool: { tool: 'git.status', input: { cwd: ctx.cwd } },
          },
          {
            label: 'Review diff',
            acceptText: 'git diff',
            tool: { tool: 'git.diff', input: { cwd: ctx.cwd } },
          },
    ...
    rules.push({
      ...
      next: {
        ...
        tool: {
          tool: 'shell.run',
          input: {
            cmd: "tail -n 50 ~/.rinawarp/agent.log 2>/dev/null || echo 'No agent logs found'",
          },
        },
      },
    });
  ...
  if (
    ctx.node?.hasPackageJson &&
    ctx.lastShell &&
    ctx.lastShell.cmd.includes('npm install') &&
    ctx.lastShell.exitCode === 0
  ) {
    rules.push({
      score: 45,
      next: {
  ...
  if (ERROR_PATTERNS.whenReady.test(stderr)) {
    results.push({
      ...
      next: {
        ...
        detail:
          'Ensure electron `app` is imported correctly and not shadowed. Check main process initialization order.',
        acceptText: 'Check main process file',
        tool: {
          tool: 'shell.run',
          input: { cmd: "find . -name 'main.js' -o -name 'main.ts' | head -5" },
        },
      },
    });
  }
    ...
    results.push({
      ...
      next: {
        ...
        tool: {
          tool: 'shell.run',
          input: {
            cmd: "env | grep -E 'RINA_AI_ENDPOINT|HTTP_PROXY|HTTPS_PROXY' && echo '--- Testing connectivity ---' && (curl -fsS \"${RINA_AI_ENDPOINT:-http://localhost}/health\" 2>/dev/null || echo 'Endpoint not reachable')",
          },
        },
      },
    });
  ...
  if (ERROR_PATTERNS.typeError.test(stderr) || ERROR_PATTERNS.syntaxError.test(stderr)) {
    results.push({
      ...
      next: {
        ...
        tool: {
          tool: 'shell.run',
          input: { cmd: "echo 'Check the error output above for file and line number'" },
        },
      },
    });
  }
