...
async function buildPlanningContext() {
  ...
  try {
    ...
    const ahead = remoteOutput.match(/ahead (\d+)/)?.[1]
      ? parseInt(remoteOutput.match(/ahead (\d+)!/)?.[1] || '0')
      : 0;
    const behind = remoteOutput.match(/behind (\d+)/)?.[1]
      ? parseInt(remoteOutput.match(/behind (\d+)!/)?.[1] || '0')
      : 0;
