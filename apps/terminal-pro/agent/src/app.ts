...
interface ChatMessage { role: Role; content: string; name?: string; tool_call_id?: string; }
interface ChatCompletionsBody {
  model: string; messages: ChatMessage[]; stream?: boolean; temperature?: number; top_p?: number; user?: string;
}
interface ApiError extends Error { status?: number; code?: string; details?: unknown; }
...
app.use((req, _res, next) => { (req as any).rid = req.headers["x-request-id"] || crypto.randomUUID(); next(); });
...
const badRequest = (message: string, details?: unknown) => Object.assign(new Error(message), { status: 400, code: "bad_request", details });
const notFound = (message: string) => Object.assign(new Error(message), { status: 404, code: "not_found" });
const problem = (s: number, c: string, m: string, d?: unknown) => Object.assign(new Error(m), { status: s, code: c, details: d });
const asUnix = () => Math.floor(Date.now() / 1000);

function requireJson(req: Request) {
  const ct = String(req.headers["content-type"] || "");
  if (!ct.startsWith("application/json")) throw problem(415, "unsupported_media_type", "content-type must be application/json");
}
function validateMessages(messages: unknown): asserts messages is ChatMessage[] {
  ...
  for (const [i, m] of messages.entries()) {
    ...
    if (!["system","user","assistant","tool","function"].includes(role)) throw badRequest(`messages[${i}].role invalid: ${String(role)}`);
...
function validateModel(model: unknown) {
  ...
  if (model !== REQUIRED_MODEL) throw badRequest(`Model not found: ${model}`, { expected: REQUIRED_MODEL });
}
function validateSampling({ temperature, top_p }: Partial<ChatCompletionsBody>) {
  if (temperature !== undefined && (typeof temperature !== "number" || temperature < 0 || temperature > 2)) throw badRequest("`temperature` must be between 0 and 2");
  if (top_p !== undefined && (typeof top_p !== "number" || top_p <= 0 || top_p > 1)) throw badRequest("`top_p` must be in (0, 1]");
}
function sseWrite(res: Response, payload: unknown) { res.write(`data: ${JSON.stringify(payload)}\n\n`); }
...
app.get("/v1/models", (_req, res) => {
  res.json({ object: "list", data: [{ id: "rina-agent", object: "model", created: asUnix(), owned_by: "local" }] });
});
...
app.post("/v1/chat/completions", asyncHandler(async (req, res) => {
  ...
  if (stream) {
    ...
    res.flushHeaders?.();
    sseWrite(res, { id: "rina-local", object: "chat.completion.chunk", created: asUnix(), model,
      choices: [{ index: 0, delta: { role: "assistant", content: "" }, finish_reason: null }] });

    let ping: NodeJS.Timeout | null = setInterval(() => res.write(`: ping ${Date.now()}\n\n`), 15000);
    const stop = () => { if (ping) clearInterval(ping); ping = null; };
    let closed = false;
    req.on("close", () => { closed = true; stop(); done(); });
    ...
    try {
      ...
      if (!closed) sseWrite(res, { id: "rina-local", object: "chat.completion.chunk", created: asUnix(), model,
        choices: [{ index: 0, delta: {}, finish_reason: "stop" }] });
    } catch (e: any) {
      sseWrite(res, { error: { message: e?.message || "stream_error", type: "stream_error" } });
    } finally {
      stop();
      if (!closed) { sseWrite(res, "[DONE]"); res.end(); }
      done();
    }
    return;
  }
...
app.post("/tool", asyncHandler(async (req, res) => {
  requireJson(req);
  if (!req.body || typeof req.body !== "object") throw badRequest("request body must be JSON object");
