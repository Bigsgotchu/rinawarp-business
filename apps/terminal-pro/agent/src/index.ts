  ...
  try {
    ...
    if (msg.type === "agent:chat") {
      ...
      if (textLower.includes("memory") || textLower.includes("remember")) {
        reply = "I'm using persistent SQLite memory! I can store and retrieve information across sessions. Try 'memory:put' or 'memory:get' tools.";
        ...
        reply = `RinaAgent (Persistent): I got "${text}". I have persistent memory and can run tools. Available tools: ${listTools().map(t => t.name).join(", ")}`;
      }
