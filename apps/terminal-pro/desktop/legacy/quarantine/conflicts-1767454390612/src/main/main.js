...
(async () => {
  ...
  if (process.argv.includes('--headless-smoke')) {
    ...
    try {
      // Test node-pty (already imported)
      ...
      // Test WebSocket server startup (already imported)
      const testServer = new WebSocket.Server({ port: 0 });
      console.log('[Smoke Test] ✓ WebSocket server started on port', testServer.address().port);
      testServer.close();

      // Test electron app functionality
      console.log('[Smoke Test] ✓ Electron app initialized');
      console.log('[Smoke Test] ✓ App version:', app.getVersion());
