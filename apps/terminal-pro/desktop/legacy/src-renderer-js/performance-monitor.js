...
class PerformanceMonitor {
    ...
    recordResourceTiming(entry) {
        ...
        if (entry.duration > 1000) {
            this.recordWarning('Slow Resource', `Resource ${entry.name} took ${entry.duration.toFixed(2)}ms to load`);
        }
    }
    ...
    checkThresholds() {
        ...
        if (latestMemory && latestMemory.used > this.thresholds.memoryCritical) {
            this.recordError('Memory Critical', `Memory usage critical: ${this.formatBytes(latestMemory.used)}`);
        } else if (latestMemory && latestMemory.used > this.thresholds.memoryWarning) {
            this.recordWarning('Memory Warning', `Memory usage high: ${this.formatBytes(latestMemory.used)}`);
        }
        ...
        if (recentResponseTimes.length > 0) {
            ...
            if (avgResponseTime > this.thresholds.responseTimeCritical) {
                this.recordError('Response Time Critical', `Average response time: ${avgResponseTime.toFixed(2)}ms`);
            } else if (avgResponseTime > this.thresholds.responseTimeWarning) {
                this.recordWarning('Response Time Warning', `Average response time: ${avgResponseTime.toFixed(2)}ms`);
            }
        }
        ...
        if (recentFrameRates.length > 0) {
            const avgFrameRate = recentFrameRates.reduce((a, b) => a + b.fps, 0) / recentFrameRates.length;
