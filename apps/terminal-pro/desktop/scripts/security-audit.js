...
class SecurityAuditor {
    ...
    async auditCodeSigning() {
        ...
        this.results.codeSigning.status = Object.values(signingStatus).some(Boolean) ? 'partial' : 'none';
    ...
    generateRecommendations() {
        ...
        const vulnCount = this.results.vulnerabilities.filter(v => v.type === 'dependency' || v.type === 'npm-audit').length;
        if (vulnCount > 0) {
