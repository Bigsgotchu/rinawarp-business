...
function testCanaryUpdateLogic() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (existing === 'canary' || existing === 'stable') {
      ...
      cohort = Math.random() < 0.10 ? 'canary' : 'stable';
  ...
  console.log(`✅ PASS: Canary assignment approximately 10%`, isApproximatelyCorrect ? 'success' : 'fail');
  ...
  console.log(`✅ PASS: Manual cohort switching`, 
...
function testCanaryStatistics() {
  ...
  console.log(`✅ PASS: Canary statistics calculation`, canaryOnlineRate >= 0 && canaryOnlineRate <= 1 ? 'success' : 'fail');
  console.log(`✅ PASS: Stable statistics calculation`, stableOnlineRate >= 0 && stableOnlineRate <= 1 ? 'success' : 'fail');
}
