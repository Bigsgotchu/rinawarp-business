...
export function createSafeModeBundle(error, stack) {
  ...
  const bundleDir = path.join(os.homedir(), '.rinawarp-terminal-pro', 'reports');
  ...
  fs.writeFileSync(path.join(bundlePath, 'meta.json'), JSON.stringify(meta, null, 2));

  // Copy recent logs if available
  const logDir = path.join(os.homedir(), '.rinawarp-terminal-pro', 'logs');
...
export function shouldEnterSafeMode() {
  const reportsDir = path.join(os.homedir(), '.rinawarp-terminal-pro', 'reports');
...
export function setupCrashHandlers() {
  ...
  process.on('uncaughtException', (error) => {
    ...
    if (CrashRecoveryManager.shouldEnterSafeMode()) {
      ...
      process.env.RINAWARP_SAFE_MODE = 'true';
    }
