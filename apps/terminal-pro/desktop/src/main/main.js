...
async function bootstrap() {
    ...
    const baseRinaClient = new CloudflareWorkerRinaClient({
        baseUrl: process.env.RINA_WORKER_BASE_URL ?? 'https://YOUR-WORKER.workers.dev',
        apiKey: process.env.RINA_WORKER_API_KEY,
    ...
    registerRinaIpc(enhancedRinaProvider);
    registerAppIpc();
    await app.whenReady();
    createMainWindow();
}
(async () => {
    if (await runSmokeTestIfRequested(process.argv))
        return;
