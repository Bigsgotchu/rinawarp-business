...
function parseSmokeTestArgs(argv) {
    if (!argv.includes('--smoke-test'))
        return null;
    const timeoutIdx = argv.findIndex((a) => a === '--smoke-timeout-ms');
    const timeoutMs = timeoutIdx >= 0 && Number.isFinite(Number(argv[timeoutIdx + 1]))
...
async function verifyIpc(win, opts) {
...
async function runSmokeTest(opts) {
    ...
    win.webContents.on('did-fail-load', (_e, code, desc, url) => fail(`did-fail-load code=${code} desc=${desc} url=${url}`));
    try {
...
export async function runSmokeTestIfRequested(argv = process.argv) {
