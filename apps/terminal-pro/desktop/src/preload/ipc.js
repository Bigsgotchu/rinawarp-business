...
const safeIpc = {
    ...
    // Billing
    billing: {
        startUpgrade: (tier, email) => ipcRenderer.invoke(IPC_CHANNELS.BILLING.START_UPGRADE, { tier, email }),
    },
    // Update
    update: {
    ...
    websocket: {
        ...
        broadcastTerminalData: (sessionId, terminalId, data) => ipcRenderer.invoke(IPC_CHANNELS.WEBSOCKET.BROADCAST_TERMINAL_DATA, {
            sessionId,
            terminalId,
            data,
        }),
    ...
    policy: {
        ...
        set: (mode) => ipcRenderer.invoke('policy:set', mode),
    },
    // Smoke test IPC
    smokeTest: {
    ...
    terminalNew: {
        ...
        proposeExec: (command, cwd, env) => ipcRenderer.invoke('terminal:propose-exec', { command, cwd, env }),
        approveExec: (terminalId, token) => ipcRenderer.invoke('terminal:approve-exec', { terminalId, token }),
    },
};
...
contextBridge.exposeInMainWorld('rinaWarp', {
    ...
    rinaSetOfflineMode: async (offline) => ipcRenderer.invoke('rina:setOfflineMode', Boolean(offline)),
    rinaSmokeRoundTrip: async (prompt, offline) => ipcRenderer.invoke('rina:smokeRoundTrip', {
        prompt: String(prompt),
        offline: Boolean(offline),
    }),
    policyGet: async () => ipcRenderer.invoke('policy:get'),
    policySet: async (mode) => ipcRenderer.invoke('policy:set', mode),
});
