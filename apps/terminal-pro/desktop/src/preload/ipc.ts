...
const safeIpc = {
  ...
  config: {
    ...
    setLicenseKey: (licenseKey: string) => ipcRenderer.invoke(IPC_CHANNELS.CONFIG.SET_LICENSE_KEY, licenseKey),
  ...
  rina: {
    ...
    setLayout: (layoutUpdate: any) => ipcRenderer.invoke(IPC_CHANNELS.RINA.SET_LAYOUT, layoutUpdate),
  },

  // Billing
  billing: {
    startUpgrade: (tier: string) => ipcRenderer.invoke(IPC_CHANNELS.BILLING.START_UPGRADE, { tier }),
  },
  ...
  // Team
  team: {
    createBillingSession: (teamId: string, seats: number) => ipcRenderer.invoke(IPC_CHANNELS.TEAM.CREATE_BILLING_SESSION, { teamId, seats }),
    getSeats: (teamId: string) => ipcRenderer.invoke(IPC_CHANNELS.TEAM.GET_SEATS, { teamId }),
    createSharedSession: (teamId: string, sessionName: string) => ipcRenderer.invoke(IPC_CHANNELS.TEAM.CREATE_SHARED_SESSION, { teamId, sessionName }),
    joinSession: (sessionId: string) => ipcRenderer.invoke(IPC_CHANNELS.TEAM.JOIN_SESSION, { sessionId }),
    getActivity: (teamId: string, limit?: number) => ipcRenderer.invoke(IPC_CHANNELS.TEAM.GET_ACTIVITY, { teamId, limit }),
    storeAiMemory: (teamId: string, memoryType: string, content: string, tags: string[]) => ipcRenderer.invoke(IPC_CHANNELS.TEAM.STORE_AI_MEMORY, { teamId, memoryType, content, tags }),
    searchMemory: (teamId: string, query: string, memoryType?: string, limit?: number) => ipcRenderer.invoke(IPC_CHANNELS.TEAM.SEARCH_MEMORY, { teamId, query, memoryType, limit }),
  },
  ...
  terminal: {
    ...
    write: (id: string, data: string) => ipcRenderer.invoke(IPC_CHANNELS.TERMINAL.WRITE, { id, data }),
    resize: (id: string, cols: number, rows: number) => ipcRenderer.invoke(IPC_CHANNELS.TERMINAL.RESIZE, { id, cols, rows }),
  ...
  websocket: {
    ...
    createSharedSession: (terminalId: string, sessionName: string) => ipcRenderer.invoke(IPC_CHANNELS.WEBSOCKET.CREATE_SHARED_SESSION, { terminalId, sessionName }),
    joinSharedSession: (sessionId: string) => ipcRenderer.invoke(IPC_CHANNELS.WEBSOCKET.JOIN_SHARED_SESSION, { sessionId }),
    broadcastTerminalData: (sessionId: string, terminalId: string, data: string) => ipcRenderer.invoke(IPC_CHANNELS.WEBSOCKET.BROADCAST_TERMINAL_DATA, { sessionId, terminalId, data }),
  },
  ...
  conversation: {
    sendMessage: (message: string) => ipcRenderer.invoke(IPC_CHANNELS.CONVERSATION.SEND_MESSAGE, message),
  ...
  filesystem: {
    ...
    writeText: (path: string, content: string) => ipcRenderer.invoke(IPC_CHANNELS.FILESYSTEM.WRITE_TEXT, path, content),
  ...
  intent: {
    ...
    executeAction: (action: string, params: any) => ipcRenderer.invoke(IPC_CHANNELS.INTENT.EXECUTE_ACTION, action, params),
  },
  ...
  terminalNew: {
    ...
    write: (id: string, data: string) => ipcRenderer.invoke(IPC_CHANNELS.TERMINAL_NEW.WRITE, id, data),
