...
export async function askRinaChat(payload) {
  try {
    ...
    if (window.aiAssistant && window.aiAssistant.isReady()) {
      const prompt =
        typeof payload === 'string' ? payload : payload.prompt || payload.message || '';
...
export async function askAboutOutput(payload = {}) {
  const { command = '', stdout = '', stderr = '', code = null, cwd = '' } = payload;

  const prompt = `You are Rina. Explain the terminal command output clearly and suggest next steps if relevant.
