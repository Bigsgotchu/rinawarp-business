...
class MemoryLeakDetector {
  ...
  aggressiveCleanup() {
    ...
    if (
      ...
      typeof window.terminalManager.performMemoryCleanup === 'function'
    ) {
  ...
  emitMemoryLeakAlert(alert) {
    ...
    window.dispatchEvent(
      new CustomEvent('memory-leak-alert', {
        detail: alert,
      }),
    );
  ...
  getMemoryReport() {
    ...
    const memoryIncrease =
      baseline && current ? current.memory.usedJSHeapSize - baseline.memory.usedJSHeapSize : 0;
