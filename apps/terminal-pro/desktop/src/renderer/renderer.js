(function () {
  ...
  const btnExport = $('btnExport'),
    btnFixPolicy = $('btnFixPolicy');
  const btnShowGraph = $('btnShowGraph'),
    graphModal = $('graphModal'),
    graphClose = $('graphClose'),
    graphSvg = $('graphSvg');
  const btnDrySel = $('btnDrySel'),
    btnExecSel = $('btnExecSel'),
    selSummary = $('selSummary');

  let lastPlan = []; // NEW: keep raw plan for visualization
  let selectedIds = new Set();

  function chipSet(el, on) {
    el.classList.toggle('tag-on', on);
    el.classList.toggle('tag-off', !on);
  }
  function getCwd() {
    return cwdEl.value && cwdEl.value.trim() ? cwdEl.value.trim() : '/';
  }
  ...
  function updateSelSummary() {
    selSummary.textContent = selectedIds.size
  ...
  async function loadCaps() {
    ...
    chipSet(capDocker, !!caps.docker);
    chipSet(capGit, !!caps.git);
    chipSet(capNpm, !!caps.npm);
    chipSet(capNetwork, !!caps.network);
  }
  ...
  async function doPlan() {
    ...
    if (res.status === 'ok') {
      renderPlan(res.plan);
      outEl.textContent = 'Review plan. Select steps if needed. F9=Dry-run · F10=Execute';
    } else {
      errEl.textContent = res.message || 'Plan failed';
    }
  }
  ...
  function topoLayers(steps) {
    ...
    for (const s of steps)
      for (const d of s.requires || []) indeg.set(s.id, (indeg.get(s.id) || 0) + 1);
    const q = [];
  ...
  function drawGraph(steps, svg) {
    ...
    for (const s of steps) {
      ...
      t1.setAttribute('y', p.y + 22);
      t1.setAttribute('font-size', '12');
      t1.setAttribute('fill', '#aaa');
      t1.textContent = s.id;
      const t2 = document.createElementNS(NS, 'text');
      t2.setAttribute('x', p.x + 12);
      t2.setAttribute('y', p.y + 40);
      t2.setAttribute('font-size', '12');
      t2.setAttribute('fill', '#ddd');
  ...
  btnDry.onclick = async () => {
    ...
    if (res.status === 'ok') {
      ...
      errEl.textContent = res.message || 'Dry-run failed';
    }
  };
  btnExec.onclick = async () => {
    ...
    if (res.status === 'ok') {
      renderPlan([]);
      outEl.textContent = res.stdout || 'Completed.';
      errEl.textContent = res.stderr || '';
    } else {
      renderPlan(res.plan || []);
      errEl.textContent = `[${res.status}] ${res.message}\n` + (res.stderr || '');
  ...
  // Updater Panel
  const btnUpdater = $('btnUpdater');
  let updaterPanel = null;

  // Initialize updater panel
  async function initUpdaterPanel() {
    if (!window.UpdaterPanel) {
      console.error('UpdaterPanel not found');
      return;
    }

    try {
      // Get current version from electron API
      const version = await window.electronAPI.getVersion();
      const feedUrl = 'https://updates.rinawarp.dev/stable'; // Default feed URL

      updaterPanel = new window.UpdaterPanel({
        container: document.body,
        currentVersion: version || '0.0.0',
        feedUrl: feedUrl,
      });
    } catch (e) {
      console.error('Failed to initialize updater panel:', e);
    }
  }
  ...
  btnUpdater.onclick = () => {
    if (!updaterPanel) {
      alert('Updater panel not initialized yet. Please try again.');
      return;
    }
    updaterPanel.show();
  };
    ...
    try {
      ...
      if (meta.version !== last) {
        ...
        if (meta.assets?.checksums) {
          wnSums.href = meta.assets.checksums.startsWith('/')
            ? meta.assets.checksums
            : `/download/checksums`;
        } else {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize updater panel after DOM is ready
    setTimeout(initUpdaterPanel, 1000);
  });

  // C# Script Smoke Test
  const btnRunCs = $('btn-run-cs');
  if (btnRunCs && window.rwTools?.csharp?.run) {
    btnRunCs.onclick = async () => {
      const code = `
using System;
public class Script {
  public static int Main(string[] args) {
    Console.WriteLine("Hello from CS-Script!");
    Console.WriteLine("Args: " + string.Join(",", args));
    return 0;
  }
}
`;
      const r = await window.rwTools.csharp.run({ code, args: ['a', 'b'], timeoutMs: 10000 });
      console.log('csharp.run ->', r);
      alert(r.ok ? 'OK\\n' + r.result.stdout : 'ERR\\n' + r.error);
    };
  }

  // Tiny Diagnostics formatter: textarea -> ANSI preview
  const csIn = document.getElementById('cs-input');
  const csBtn = document.getElementById('btn-format-cs');
  const csOut = document.getElementById('cs-output');

  if (csBtn && csIn && csOut && window.rwTools?.csharp?.format) {
    csBtn.addEventListener('click', async () => {
      const code = csIn.value || '';
      const r = await window.rwTools.csharp.format({ code });
      const text = r?.result?.ansi || code;
      csOut.textContent = text; // keep ANSI; your terminal-view should render it
    });
  }
})();
