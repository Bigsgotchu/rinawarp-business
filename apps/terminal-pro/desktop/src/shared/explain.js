function explainStep(step) {
  ...
  if (/source .venv\/bin\/activate/.test(c)) return 'Activates the virtual environment so python/pip refer to .venv.';
  if (/\bpip install pytest\b/.test(c)) return 'Installs pytest for running tests.';
  if (/docker build/.test(c)) return 'Builds a Docker image named app:latest from the current directory.';
