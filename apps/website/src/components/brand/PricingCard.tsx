...
export const PricingCard: React.FC<PricingCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl md:text-2xl font-semibold text-brand-text mb-2">{name}</h3>
      <p className="text-sm text-brand-muted mb-4">{description}</p>

      <div className="flex items-baseline gap-1 mb-6">
        <span className="text-3xl md:text-4xl font-bold text-brand-text">{price}</span>
        <span className="text-xs uppercase tracking-widest text-brand-muted">{cadence}</span>
      </div>
