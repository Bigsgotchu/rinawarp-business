...
async function promoteCanaryToStable(version) {
  try {
    ...
    const stableYmlContent = canaryYmlContent.replace(/version: .*/, `version: ${version}`);
    const stableMacYmlContent = canaryMacYmlContent.replace(/version: .*/, `version: ${version}`);
...
function canaryMeetsPromotionCriteria(canary, stable) {
  ...
  if (onlineRateDiff > ONLINE_RATE_DIFF_THRESHOLD) {
    console.log(
      `❌ Canary online rate too low: ${canary.agentOnlineRate} vs stable ${stable.agentOnlineRate} (diff: ${onlineRateDiff})`,
    );
    return false;
  }
  ...
  if (crashRateDiff > CRASH_RATE) {
    console.log(
      `❌ Canary crash rate too high: ${canary.crashRate} vs stable ${stable.crashRate} (diff: ${crashRateDiff})`,
    );
    return false;
  }
...
export async function runCanaryPromotion() {
  try {
    ...
    console.log(
      `📊 Canary: ${canary.sampleCount} samples, ${(canary.agentOnlineRate * 100).toFixed(1)}% online, ${(canary.crashRate * 100).toFixed(3)}% crash`,
    );
    console.log(
      `📊 Stable: ${stable.sampleCount} samples, ${(stable.agentOnlineRate * 100).toFixed(1)}% online, ${(stable.crashRate * 100).toFixed(3)}% crash`,
    );
    ...
    const msg =
      `✅ Canary eligible for promotion: ${latestCanaryVersion}\n` +
