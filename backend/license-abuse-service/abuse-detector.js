...
export function processLicenseEvent(licenseKey, deviceId, ip, outcome) {
  ...
  const recentEvents = licenseEvents.get(licenseKeyHash)
  ...
  if (shouldQuarantine) {
    ...
    console.log(`🔐 License quarantined: ${licenseKeyHash.substring(0, 8)}... (Score: ${abuseScore})`);
  } else if (shouldClear) {
...
export function getAbuseStatistics() {
  ...
  const quarantinedLicenses = Array.from(licenseStates.values())
    .filter(state => state.quarantined).length;
  
  // Calculate cleared in last 24h
  const now = new Date();
  const last24h = new Date(now.getTime() - 24 * 60 * 60 * 1000);
  const cleared24h = Array.from(licenseStates.values())
    .filter(state => 
      state.quarantined && 
      state.quarantinedAt && 
  ...
  return {
    ...
    quarantineRate: totalLicenses > 0 ? (quarantinedLicenses / totalLicenses * 100).toFixed(2) : '0.00'
  };
}
