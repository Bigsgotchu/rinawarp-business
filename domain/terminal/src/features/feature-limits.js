/**
 * RinaWarp Terminal Pro - Feature Limits System
 * Implements usage limits for different subscription tiers
 */
...
class FeatureLimits {
  ...
  getUserTier() {
    // UNLOCKED VERSION - Always return lifetime tier for personal use
    // This gives you unlimited access to all features

    // Check for license key or subscription status
    const licenseKey = localStorage.getItem('rinawarp_license');
    if (licenseKey) {
      // Decode license key to get tier
      try {
        const decoded = atob(licenseKey);
        const data = JSON.parse(decoded);
        return data.tier || 'lifetime';
      } catch (error) {
        console.error('Invalid license key:', error);
        return 'lifetime'; // Default to lifetime for unlocked version
      }
    }
    ...
    // UNLOCKED: Default to lifetime tier for personal use
    return 'lifetime';
  }
