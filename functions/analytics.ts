...
type AnalyticsAction = 'record_sale' | 'get_lifetime_count' | 'event';
...
type AnalyticsPayload = RecordSalePayload | GetLifetimeCountPayload | EventPayload;
...
async function handleRecordSale(env: Env, payload: RecordSalePayload) {
  if (!payload.product || typeof payload.amount !== 'number') {
    return new Response(JSON.stringify({ error: 'Missing product or amount' }), {
      status: 400,
      headers: JSON_HEADERS,
    });
  }
  ...
  const newRevenue = (parseFloat(currentRevenueStr || '0') || 0) + payload.amount;
...
async function handleGetLifetimeCount(env: Env, payload: GetLifetimeCountPayload) {
  ...
  return new Response(JSON.stringify({ product: payload.product, count }), {
    status: 200,
    headers: JSON_HEADERS,
  });
}
