...
(function (window, document) {
  ...
  function updateSeatUI(remaining, total) {
    ...
    var pct = safeTotal > 0 ? Math.max(0, Math.min(100, (remaining / safeTotal) * 100)) : 0;
  ...
  function initSeatCounter() {
    ...
    function fetchSeats() {
      ...
      safeFetchJSON(SEAT_ENDPOINT)
        .then(function (data) {
          var remaining = typeof data.remaining === 'number' ? data.remaining : total;
    ...
    safeFetchJSON(RINA_API_BASE + '/api/files/tree')
      .then(function (data) {
        ...
        data.items.forEach(function (item) {
          byPath[item.path] = Object.assign({}, item, { children: item.children || [] });
        });
      ...
      .catch(function (err) {
        ...
        container.innerHTML = '<p class="rw-file-tree-error">Unable to load file tree</p>';
      });
  }
  ...
  function initEmailSignup() {
    ...
    function showMessage(message, type) {
      var messageEl = form.querySelector('.rw-form-message') || document.createElement('div');
    ...
    function submitEmail(email) {
      ...
      .then(function(data) {
        showMessage('🎉 You\'re in! Check your inbox for confirmation.', 'success');
        emailInput.value = '';
