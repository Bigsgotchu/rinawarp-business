...
function main() {
  ...
  const file = expand(
    args.get('--config')?.[0] || path.join(os.homedir(), '.continue', 'config.yaml'),
  );
  const blocksRoot = expand(
    args.get('--blocks-root')?.[0] || process.env.BLOCKS_ROOT || DEFAULT_BLOCKS_ROOT,
  );
  ...
  if (scaffoldFlag) {
    for (const slug of finalSlugs) {
      const wasMissing =
        ...
        (fallback && Object.values(map).includes(slug) && !localBlockExists(slug, blocksRoot));
