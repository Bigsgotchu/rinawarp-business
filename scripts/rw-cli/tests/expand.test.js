...
describe('RW Task Expander', () => {
  it('should show original task and expanded steps', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Validate system dependencies'),
    );
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Draft solution outline'));
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Execute Step 1 of implementation'),
    );
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Request confirmation before Step 2'),
    );
