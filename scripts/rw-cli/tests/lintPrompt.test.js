...
describe("RW Prompt Linter", () => {
  ...
  it("should detect missing punctuation", () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Prompt has no clear end punctuation"));
    consoleSpy.mockRestore();
  });
  ...
  it("should detect mixed repair and build tasks", () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Mixing repair and build tasks"));
    consoleSpy.mockRestore();
  });
