...
# Uses pandoc if available, otherwise simple conversion
...
echo "🚀 Starting Simple Markdown to HTML Conversion..."
...
for md_file in "${!DOCUMENT_MAP[@]}"; do
    ...
    if [[ -f "${LEGAL_DOCS_SOURCE}/${md_file}" ]]; then
        ...
        # Simple HTML conversion - wrap content in basic HTML tags
        html_content="<h1>${doc_title}</h1>"
        html_content+=$'\n'$(echo "$markdown_content" | tail -n +2 | sed \
            -e 's/^## /<h2>/g' \
            -e 's/^### /<h3>/g' \
            -e 's/^#### /<h4>/g' \
            -e 's/^> /<blockquote>/g' \
...
echo "   - Successfully converted ${converted_count} documents"
