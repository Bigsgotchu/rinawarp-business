...
# Simple Legal Document Page Generator
# Creates basic HTML pages for legal documents
...
declare -A DOCUMENT_MAP=(
    ...
    ["ETHICAL_AI_POLICY.md"]="ai-ethical-policy.html"
...
for md_file in "${!DOCUMENT_MAP[@]}"; do
    ...
    if [[ -f "${LEGAL_DOCS_SOURCE}/${md_file}" ]]; then
        ...
        # Extract title from first line (simple approach)
        doc_title=$(echo "$markdown_content" | head -1 | sed 's/^# //')
        doc_description="Legal document: ${doc_title} for RinaWarp Technologies LLC"

        # Create HTML file using simple string replacement
        final_html="${HTML_TEMPLATE}"
        final_html=${final_html//"DOC_TITLE"/"$doc_title"}
        final_html=${final_html//"DOC_DESCRIPTION"/"$doc_description"}
        final_html=${final_html//"DOC_CONTENT"/"$markdown_content"}
