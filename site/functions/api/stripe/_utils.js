export const json = (data, init={}) => new Response(JSON.stringify(data), { headers: { 'content-type': 'application/json' }, ...init });
...
export function okOrThrow(res) { if (!res.ok) throw new Error(`Stripe ${res.status}`); return res; }

export function originFromEnv(env) { return (env.SITE_BASE || '').replace(/\/$/,''); }
