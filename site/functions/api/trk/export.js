export const onRequestGet = async ({ request, env }) => {
  try {
    ...
    if (!day || !/^\d{4}-\d{2}-\d{2}$/.test(day)) {
      return new Response(JSON.stringify({ ok: false, error: "day param required as YYYY-MM-DD" }), {
    ...
    if (!raw) {
      return new Response(format === 'json' ? JSON.stringify({ ok: true, data: [] }) : "t,d,ev,path,btn,href,utm_source,utm_medium,utm_campaign,ref,ua\n", {
        status: 200,
        headers: { "content-type": format === 'json' ? "application/json" : "text/csv; charset=utf-8",
    ...
    const rows = events.map(e =>
      [e.t, e.d, e.ev, e.path, e.btn, e.href, e.utm_source, e.utm_medium, e.utm_campaign, e.ref, e.ua]
        .map(esc).join(",")
    );
