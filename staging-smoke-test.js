...
// Configuration
// ===============================
const DEPLOYMENT_URL = process.env.DEPLOYMENT_URL || "https://rinawarp-website-staging.pages.dev";
const PLANS = ["basic", "starter", "creator", "enterprise"];
const TIMEOUT = 10000; // ms for staging
...
async function testAPIHealth() {
  try {
    const controller = new AbortController();
    const id = setTimeout(() => controller.abort(), TIMEOUT);
    const res = await fetch(`${DEPLOYMENT_URL}/api/health`, { method: "GET", signal: controller.signal });
    clearTimeout(id);
...
async function testPlanCheckout(plan) {
  try {
    const controller = new AbortController();
    const id = setTimeout(() => controller.abort(), TIMEOUT);
    const res = await fetch(`${DEPLOYMENT_URL}/api/checkout-v2`, {
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
        "User-Agent": "RinaWarp-Staging-Smoke-Test/1.0"
      },
      body: JSON.stringify({ 
        plan,
        test_mode: true,
        source: "staging_smoke_test"
      }),
...
async function checkPerformance() {
  try {
    ...
    await fetch(DEPLOYMENT_URL, { method: "HEAD" });
...
// ===============================
(async () => {
  console.log("\x1b[36m[STAGING SMOKE TEST]\x1b[0m === RinaWarp Staging Smoke Test ===\n");
  console.log(`Target: ${DEPLOYMENT_URL}\n`);

  // 1️⃣ Check Website Accessibility (Critical)
  console.log("\x1b[36m[STAGING SMOKE TEST]\x1b[0m 1. Checking website accessibility...");
  testResults.websiteAccessibility = await withRetry(
    () => checkServer(DEPLOYMENT_URL, "Website accessibility", true),
    "Website accessibility check"
  );

  // 2️⃣ Check API Health (Critical)
  if (testResults.websiteAccessibility) {
    console.log("\x1b[36m[STAGING SMOKE TEST]\x1b[0m 2. Checking API health...");
    testResults.apiHealth = await withRetry(
      testAPIHealth,
      "API health check"
    );
  } else {
    console.warn("\x1b[36m[STAGING SMOKE TEST]\x1b[0m ⚠️ Skipping API health check due to website accessibility failure\n");
  }

  // 3️⃣ Test Checkout Flows (Critical)
  if (testResults.apiHealth) {
    console.log("\x1b[36m[STAGING SMOKE TEST]\x1b[0m 3. Testing checkout flows for all plans...");
    for (const plan of PLANS) {
      await testPlanCheckout(plan);
    }
  } else {
    console.warn("\x1b[36m[STAGING SMOKE TEST]\x1b[0m ⚠️ Skipping checkout tests due to API health failure\n");
  }

  // 4️⃣ Security Checks (Non-Critical)
  console.log("\x1b[36m[STAGING SMOKE TEST]\x1b[0m 4. Running security checks...");
  await validateSSL();
  await checkCSPHeaders();

  // 5️⃣ Performance Check (Non-Critical)
  console.log("\x1b[36m[STAGING SMOKE TEST]\x1b[0m 5. Basic performance check...");
  ...
  // ==============================
  console.log("\n" + "=".repeat(60));
  console.log("\x1b[36m[STAGING SMOKE TEST]\x1b[0m TEST RESULTS SUMMARY");
  ...
  console.log(`\n⚡ Performance: ${testResults.performance || 'unknown'}`);
