...
class AITester {
    ...
    async testAIInitialization() {
        ...
        try {
            ...
            const aiRouterPath = path.join(__dirname, 'apps/terminal-pro/desktop/src/renderer/js/ai-router.js');
            ...
            if (fs.existsSync(aiRouterPath)) {
                ...
                if (routerCode.includes('MultiModelAIRouter') &&
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                try {
                    ...
                    if (routerCode.includes('LocalProvider') && routerCode.includes('http://localhost:11434')) {
                        resolve({
                            success: true,
            ...
            setTimeout(() => {
                try {
                    ...
                    if (input.toLowerCase().includes('hello')) {
                        ...
                        response = 'You are using RinaWarp Terminal Pro version 1.0.0. This is the latest stable release with advanced AI capabilities.';
                    } else if (input.toLowerCase().includes('ls')) {
                        response = 'The `ls` command lists directory contents. In your current directory, you would typically see files and folders. Would you like me to explain specific ls options?';
                    } else if (input.toLowerCase().startsWith('ai:')) {
                        response = 'I can help with various tasks including: code explanation, command suggestions, error debugging, workflow optimization, and general programming assistance.';
                    } else {
                        response = 'I received your input and can provide helpful information about terminal usage, coding, and RinaWarp Terminal Pro features.';
                    }
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                try {
                    ...
                    if (aiCode.includes('getCommandSuggestions') && aiCode.includes('parseCommandSuggestions')) {
                        resolve({
                            success: true,
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                try {
                    ...
                    const hasErrorHandling = aiRouterCode.includes('handleApiError') &&
                                           aiCode.includes('catch (error)');
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                try {
                    ...
                    const hasNetworkErrorHandling = aiRouterCode.includes('Network error') ||
                                                 aiRouterCode.includes('fetch');

                    resolve({
                        hasNetworkError: !hasNetworkErrorHandling,
                        message: hasNetworkErrorHandling ? 'Network error handling found' : 'Network error handling missing'
                    });
    ...
    generateTestReport() {
        ...
        const { aiAgentStarts, backendConnectivity, receivesResponses,
                handlesCommands, noCORSErrors, noNetworkErrors,
                noAuthErrors, noBlankResponses, errors } = this.testResults;
    ...
    calculateScore() {
        const { aiAgentStarts, backendConnectivity, receivesResponses,
                handlesCommands, noCORSErrors, noNetworkErrors,
                noAuthErrors, noBlankResponses } = this.testResults;
