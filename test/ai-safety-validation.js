  ...
  createSafePlan(goal, context) {
    ...
    if (riskySteps.length > 0) {
      steps.unshift({
        ...
        args: {
          message:
            'The AI has identified potential issues. Review the suggested steps and approve execution.',
...
function testV1SafeRules() {
  ...
  logTest(
    ...
    allDangerousDetected
      ? '✅ All dangerous commands flagged'
      : '❌ Some dangerous commands missed',
  );
...
function testPlanGenerationSafety() {
  ...
  const allStepsValid = plan.steps.every(
    (step) => step.intent && step.tool && step.expectedSignal && step.explanation !== undefined,
  );
  ...
  const riskLevelsCorrect =
    lowRiskPlan.riskLevel === 'low' && ['low', 'medium'].includes(mediumRiskPlan.riskLevel);
...
function testErrorHandling() {
  ...
  logTest(
    ...
    allContextsHandled,
    allContextsHandled
      ? '✅ Fallback works for all contexts'
      : '❌ Fallback fails for some contexts',
  );
...
function testFeatureCompleteness() {
  ...
  logTest(
    ...
    validStepCount,
    validStepCount
      ? `✅ Generated ${stepCount} steps`
      : `❌ Generated ${stepCount} steps (out of range)`,
  );
  results.push(validStepCount);

  // Test 5.2: Each step has tool + args + expected signal
  const allStepsComplete = plan.steps.every(
    (step) => step.tool && step.args !== undefined && step.expectedSignal,
  );
