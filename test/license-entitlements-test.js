...
async function testEntitlementMapping() {
  ...
  const freeHasNoPremium = !freeLicense.entitlements.terminal_pro_lifetime && 
                          freeLicense.entitlements.basic_terminal;
...
async function testLicenseActivationSecurity() {
  ...
  const rateLimitTest = () => {
    ...
    const isRateLimited = (ip) => {
      ...
      const recentAttempts = attempts.filter(a => 
        a.ip === ip && (now - a.timestamp) < WINDOW_MS
      );
  ...
  const deviceFingerprintTest = () => {
    ...
    const isHashed = license1.device_fingerprint.includes('hash') || 
                    license1.device_fingerprint.length > 20;
