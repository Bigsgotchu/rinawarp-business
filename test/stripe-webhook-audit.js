...
for (const rel of WEBHOOK_CANDIDATES) {
  ...
  if (bad) {
    fail(
      `Duplicate webhook verification detected in ${rel}`,
      'Move webhook verification to backend/stripe-secure/webhook.js only.',
    );
  } else {
...
banner('🔍 Environment Configuration');
{
  ...
  for (const key of required) {
    if (!s.includes(key)) {
      fail(
        `Canonical webhook does not reference ${key}`,
        'Add a runtime guard that throws if missing.',
      );
    } else {
...
banner('🔍 Subscription Event Handling');
{
  ...
  if (missing.length) {
    fail(
      'Canonical webhook missing subscription-related event handling',
