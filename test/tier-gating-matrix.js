...
function logMatrix(testName, expected, actual, passed) {
  ...
  if (!passed) {
    console.log(
      `   Expected: Terminal Pro=${expected.terminalPro}, Agent Pro=${expected.agentPro}`,
    );
...
class FeatureGate {
  ...
  isFeatureAccessible(feature) {
    switch (feature.tier) {
      ...
      case 'terminal-pro':
        return (
          this.entitlements.terminal_pro_lifetime === true ||
...
function testTierCombination(testCase) {
  ...
  const terminalFeatureTests = testCase.expected.terminalProFeatures.map((featureName) => {
    ...
    logTest(
      `Terminal Pro Feature: ${featureName}`,
      hasFeature === shouldHave,
      hasFeature ? '✅ Accessible' : '❌ Not accessible',
    );
  ...
  if (testCase.expected.periodActive) {
    ...
    logTest(
      ...
      periodStillActive
        ? '✅ Period still active, features maintained'
        : '❌ Period end not handled',
    );
...
function testEdgeCases() {
  ...
  logTest(
    ...
    handlesMalformed,
    handlesMalformed
      ? '✅ Gracefully handles malformed entitlements'
      : '❌ Crashes on malformed entitlements',
  );
