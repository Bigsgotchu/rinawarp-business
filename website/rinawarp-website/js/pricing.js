(() => {
  ...
  function applyLifetimeVisibility(pricing) {
    ...
    if (soldOutEl) soldOutEl.style.display = allSoldOut ? "block" : "none";
    if (offersEl) offersEl.style.display = hasAnyAvailable ? "grid" : "none";
    ...
    for (const plan of lifetime) {
      ...
      if (el) el.style.display = plan.soldOut ? "none" : "block";
    }
  }
